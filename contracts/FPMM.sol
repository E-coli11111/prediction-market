...
import { ERC20 } from "@gnosis.pm/conditional-tokens-market-makers/contracts/ERC20.sol";
...
contract FPMM is ERC20, ERC1155TokenReceiver {
    ...
    uint public withdrawnManagerFee;
    ...
    function setManagerFee(uint _managerFee) external {
        if(managerFee == 0)
    ...
    function withdrawMangerFee() public onlyOwner {
        uint withdrawableAmount = managerfeePoolWeight - withdrawnManagerFee;
        if(withdrawableAmount > 0){
            withdrawnManagerFee = withdrawnManagerFee.add(withdrawableAmount);
            require(collateralToken.transfer(_owner, withdrawableAmount), "withdrawal transfer failed");
        }
    }
    ...
    function calcBuyAmount(uint investmentAmount, uint outcomeIndex) public view returns (uint) {
        ...
        uint[] memory poolBalances = getPoolBalances();
        uint investmentAmountMinusFees = investmentAmount.sub(investmentAmount.mul(fee + managerFee) / ONE);
    ...
    function calcSellAmount(uint returnAmount, uint outcomeIndex) public view returns (uint outcomeTokenSellAmount) {
        ...
        uint[] memory poolBalances = getPoolBalances();
        uint returnAmountPlusFees = returnAmount.mul(ONE) / ONE.sub(fee + managerFee);
    ...
    function buy(uint investmentAmount, uint outcomeIndex, uint minOutcomeTokensToBuy) external {
        require(block.timestamp < closeTime, "FPMM closed");
        ...
        uint managerfeeAmount = investmentAmount.mul(managerFee) / ONE;
        managerfeePoolWeight = managerfeePoolWeight.add(managerfeeAmount);

        uint investmentAmountMinusFees = investmentAmount.sub(feeAmount).sub(managerfeeAmount);
    ...
    function sell(uint returnAmount, uint outcomeIndex, uint maxOutcomeTokensToSell) external {
        ...
        // require(false, "test");没问题

        uint feeAmount = returnAmount.mul(fee) / (ONE.sub(fee).sub(managerFee));
        feePoolWeight = feePoolWeight.add(feeAmount);

        uint managerfeeAmount = returnAmount.mul(managerFee) / (ONE.sub(fee).sub(managerFee));
        managerfeePoolWeight = managerfeePoolWeight.add(managerfeeAmount);

        uint returnAmountPlusFees = returnAmount.add(feeAmount).add(managerfeeAmount);

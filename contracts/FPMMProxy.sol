...
contract FPMMProxy is Ownable, ERC1155, IERC1155Receiver{
    ...
    constructor(address addr) ERC1155(""){
        collateralToken = IERC20(addr);
        
    }

    function buy(uint sponsorID, address fpmm, uint investAmount, uint outcomeIndex, uint minOutcomeTokensToBuy) external{
        require(block.timestamp < deadlines[sponsorID], "Token for sponsor has expired");
        require(register[fpmm] == sponsorID, "FPMM not registered for sponsor.");
        require(!isRedeemable[fpmm], "Cannot transact during the redeem period");
        IFPMM FPMM = IFPMM(fpmm);
        _burn(msg.sender, sponsorID, investAmount);
        uint outcomeTokensToBuy = FPMM.calcBuyAmount(investAmount, outcomeIndex);
        collateralToken.approve(fpmm, investAmount);
        FPMM.buy(investAmount, outcomeIndex, minOutcomeTokensToBuy);
        // require(false, "test");
        conditionalTokenBalances[FPMM.getPositionId(outcomeIndex)][msg.sender] += outcomeTokensToBuy;
    }

    function sell(uint sponsorID, address fpmm, uint returnAmount, uint outcomeIndex, uint maxOutcomeTokensToSell) external{
        require(block.timestamp < deadlines[sponsorID], "Token for sponsor has expired");
        require(register[fpmm] == sponsorID, "FPMM not registered for sponsor.");
        require(!isRedeemable[fpmm], "Cannot transact during the redeem period");
        IFPMM FPMM = IFPMM(fpmm);
        uint outcomeTokensToSell = FPMM.calcSellAmount(returnAmount, outcomeIndex);
        require(conditionalTokenBalances[FPMM.getPositionId(outcomeIndex)][msg.sender] >= outcomeTokensToSell, "No enough outcome token.");
        conditionalTokenBalances[FPMM.getPositionId(outcomeIndex)][msg.sender] -= outcomeTokensToSell;
        // IConditionalTokens(FPMM.getConditionalTokens()).safeTransferFrom(msg.sender, address(this), FPMM.getPositionId(outcomeIndex), outcomeTokensToSell,"");
        IConditionalTokens(FPMM.getConditionalTokens()).setApprovalForAll(fpmm, true);
        FPMM.sell(returnAmount, outcomeIndex, maxOutcomeTokensToSell);
        _mint(msg.sender, sponsorID, returnAmount, "");
    }

    function registerFPMM(uint sponsorID, address fpmm) onlyOwner external{
        if(sponsorID != 0){
            require(IFPMM(fpmm).getCollateral() == address(collateralToken), "Collateral token must be the same");
        }
        register[fpmm] = sponsorID;
    }

    function redeem(address fpmm) external{
        require(isRedeemable[fpmm], "Cannot redeem yet");
        IFPMM FPMM = IFPMM(fpmm);
        ...
        require(collateralToken.transfer(msg.sender, tokenToPay), "Collateral transfer fail");

    }

    function setRedeem(address fpmm, bool status) onlyOwner external {
        require(register[fpmm] != 0, "FPMM not registered for sponsor.");
        if(status){
...
//Toy token 0x60cDD29c0a8B46B9d86AB1574CD658c0B0Ac53ce
//FPMMProxy 0x9c4094Ad26469a78C4181eE4d17DC8D12178E35c
//Condtional 0x23f3bB64246b56d7942caDD680AfB8386b84a1Dd
//Factory 0x5f4EB5cb65fE7728154e3b210dC1620e3d72101D
//Condition 0xa0db3510174f1d7e18467f68ef12b9c62c6a817186d13ca953cdd8f69e7e4a3b
//fpmm5 0x7a13251D67d8aa5966899d8e480f75102C94cc87
//0x73aAd87B07Dde9925252e3BAaE892Cb3e2dcE76c